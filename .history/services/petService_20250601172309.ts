...
class PetService {
  ...
  async addPet(petData: Omit<Pet, 'id' | 'createdAt'>): Promise<Pet> {
    try {
      ...
      // Backend'in beklediği format için ID'leri alıyoruz
      const animalTypeId = ANIMAL_TYPE_IDS[petData.species as keyof typeof ANIMAL_TYPE_IDS];
      ...
      // Backend'in beklediği formatı hazırlayalım
      const backendFormat = {
        name: petData.name,
        age: petData.age,
