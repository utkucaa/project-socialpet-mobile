...
export const analyzeDogBreed = async (imageUri: string): Promise<BreedDetectionResult> => {
  try {
    ...
    console.log('📍 API URL:', 'http://192.168.1.120:8080/api/v1/dog-breed-analyzer/analyze-dog');
    ...
    const response = await fetch('http://192.168.1.120:8080/api/v1/dog-breed-analyzer/analyze-dog', {
      method: 'POST',
      headers: {
    ...
    if (error instanceof Error && (
      ...
      console.warn('🚨 Backend API çalışmıyor olabilir. Mock veri kullanılıyor.');
    }
    ...
    // Create mock top predictions for dogs
    const mockPredictions: { [key: string]: number } = {};
    const shuffledBreeds = [...dogBreeds].sort(() => 0.5 - Math.random()).slice(0, 3);
    mockPredictions[randomBreed] = confidence / 100;
    shuffledBreeds.forEach((breed, index) => {
      if (breed !== randomBreed) {
        mockPredictions[breed] = (confidence - 10 - index * 5) / 100;
      }
    });
    
    return {
      type: 'Köpek',
      breed: `${randomBreed} (Mock)`,
      confidence: confidence,
      topPredictions: mockPredictions,
      primaryBreed: randomBreed
    };
  }
};
...
export const analyzeCatBreed = async (imageUri: string): Promise<BreedDetectionResult> => {
  try {
    ...
    console.log('📍 API URL:', 'http://192.168.1.120:8080/api/v1/breed-analyzer/analyze');
    ...
    const response = await fetch('http://192.168.1.120:8080/api/v1/breed-analyzer/analyze', {
      method: 'POST',
      headers: {
        'Content-Type': 'multipart/form-data',
      },
...
export const checkDogAnalyzerHealth = async (): Promise<boolean> => {
  try {
    const response = await fetch('http://192.168.1.120:8080/api/v1/dog-breed-analyzer/health');
    return response.ok;
  } catch (error) {
...
export const checkPythonApiHealth = async (): Promise<boolean> => {
  try {
    const response = await fetch('http://192.168.1.120:5012/health');
    return response.ok;
  } catch (error) {
