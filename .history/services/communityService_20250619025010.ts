...
const BASE_URL = 'https://socialpet-api.onrender.com/api';
...
class CommunityService {
  ...
  // Tüm soruları getir (Authentication gerektirmez)
  ...
  // Belirli bir soruyu getir (Authentication gerektirmez)
  async getQuestion(id: string): Promise<Question> {
    try {
      const response = await apiClient.get(`/questions/${id}`);
      return response.data;
    } catch (error: any) {
      console.error('Question detail API error:', error.response?.data || error.message);
      throw new Error('Soru detayları yüklenirken bir hata oluştu');
    }
  }

  // Belirli bir sorunun cevaplarını getir (Authentication gerektirmez)
  async getAnswers(questionId: string): Promise<Answer[]> {
    try {
      const response = await apiClient.get(`/answers/question/${questionId}`);
      return response.data;
    } catch (error: any) {
      console.error('Answers API error:', error.response?.data || error.message);
      throw new Error('Cevaplar yüklenirken bir hata oluştu');
    }
  }

  // Yeni soru oluştur (Authentication gerektirir)
  async createQuestion(questionData: Omit<Question, 'id' | 'createdAt' | 'updatedAt' | 'user'>): Promise<Question> {
    try {
      ...
      console.error('Create question API error:', error.response?.data || error.message);
      throw new Error('Soru eklenirken bir hata oluştu');
    }
  }

  // Soruya cevap ver (Authentication gerektirir)
  async createAnswer(questionId: string, answerData: { content: string; userId: number }): Promise<Answer> {
    try {
      ...
      console.error('Create answer API error:', error.response?.data || error.message);
