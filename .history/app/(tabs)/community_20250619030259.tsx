...
export default function CommunityScreen() {
  ...
  const loadQuestions = async () => {
    try {
      ...
      // Basit hata yönetimi - kullanıcıya göstermek yerine console'a log
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <View style={styles.questionFooter}>
        <Text style={styles.userInfo}>
          {item.user ? `${item.user.firstName} ${item.user.lastName}` : 'Anonim'} tarafından
        </Text>
        <Text style={styles.dateInfo}>
          {new Date(item.datePosted || item.createdAt || '').toLocaleDateString('tr-TR')}
        </Text>
      </View>
    </TouchableOpacity>
  );
