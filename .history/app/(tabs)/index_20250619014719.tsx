...
export default function HomeScreen() {
  ...
  const loadRecommendedItems = async () => {
    try {
      ...
      const lostItems: RecommendedItem[] = lostPets
        .slice(0, 3)
        .filter(pet => pet.id !== undefined)
        .map((pet: LostPet) => ({
          id: pet.id!,
          title: `${pet.petName} (${pet.breed || pet.animalType})`,
          image: pet.imageUrl || 'https://via.placeholder.com/200x150/8B4513/FFFFFF?text=🐱',
          type: 'lost' as const,
          location: `${pet.city}, ${pet.district} - ${pet.lastSeenLocation}`,
          data: pet
        }));
