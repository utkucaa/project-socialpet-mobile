...
export default function LostScreen() {
  ...
  const validateCurrentStep = (): boolean => {
    switch (currentStep) {
      case 1:
        return Boolean(formData.petName && formData.animalType && formData.city);
      case 2:
        return Boolean(formData.description && formData.contactName && formData.contactPhone && formData.lastSeenLocation);
      case 3:
        return true; // Fotoğraf opsiyonel olabilir
      default:
        return true;
    }
  };
  ...
  const renderListingCard = ({ item }: { item: LostPet }) => {
    ...
    return (
      ...
      >
        <Image source={getImageSource()} style={styles.listingImage} />
        <View style={styles.listingInfo}>
          <Text style={styles.listingTitle}>{item.petName}</Text>
          <Text style={styles.listingDetails}>
            {item.animalType} • {item.city}, {item.district}
          </Text>
            ...
            <View style={styles.row}>
              ...
              <View style={styles.halfWidth}>
                ...
                <Picker
                  ...
                  style={styles.picker}
                  enabled={Boolean(formData.city)}
                >
                  <Picker.Item label={formData.city ? "Seçiniz..." : "Önce şehir seçin"} value="" />
