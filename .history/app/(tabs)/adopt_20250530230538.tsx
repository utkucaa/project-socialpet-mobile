...
export default function AdoptScreen() {
  ...
  const handleSearch = () => {
    console.log('Arama yapılıyor:', searchFilter);
    
    let filtered = [...listings];
    
    // Pet türüne göre filtrele
    if (searchFilter.petType && searchFilter.petType !== '') {
      filtered = filtered.filter(listing => 
        listing.animalType.toLowerCase().includes(searchFilter.petType.toLowerCase())
      );
    }
    
    // Şehre göre filtrele
    if (searchFilter.city && searchFilter.city !== '') {
      filtered = filtered.filter(listing => 
        listing.city.toLowerCase().includes(searchFilter.city.toLowerCase())
      );
    }
    
    console.log('Filtrelenmiş sonuçlar:', filtered.length);
