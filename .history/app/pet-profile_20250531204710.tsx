...
export default function PetProfileScreen() {
  ...
  const renderCategoryButton = (item: HealthCategory) => {
    ...
    return (
      ...
      >
        <LinearGradient
          colors={isSelected ? ['#1E40AF', '#1D4ED8'] : ['#FFFFFF', '#F8FAFC']} // Dark navy blue when selected, white when not
          style={styles.categoryGradient}
        >
          <View style={styles.categoryIconContainer}>
            <Text style={[styles.categoryIcon, { color: isSelected ? '#FFFFFF' : '#6B7280' }]}>
              {item.icon}
            </Text>
          </View>
          <Text style={[styles.categoryTitle, { color: isSelected ? '#FFFFFF' : '#374151' }]}>
            {item.title}
          </Text>
        </LinearGradient>
      </TouchableOpacity>
