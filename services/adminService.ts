...
const adminService = {
  ...
  getActiveDonationOrganizations: async (): Promise<DonationOrganization[]> => {
    try {
      ...
      // Test için geçici token ekle (eğer token yoksa)
      const currentToken = await storage.getToken();
      if (!currentToken) {
        console.log('Test için geçici token ekleniyor...');
        // Bu sadece test içindir - gerçek uygulamada login ekranından gelecek
        await storage.setToken('test-token-for-donation-api');
      }
      ...
      if (error.response) {
        ...
        if (error.response.status === 404) {
          ...
          console.warn('403 hatası alındı - API public değil veya token geçersiz, mock data döndürülüyor');
      ...
      // Diğer hatalar için de mock data döndür
      console.log('API erişim hatası, mock data döndürülüyor');
      return adminService.getMockDonationOrganizations();
    }
  },
